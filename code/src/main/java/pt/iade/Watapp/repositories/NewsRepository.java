...
public interface NewsRepository extends CrudRepository<News, Integer> {
  ...
  String useQuery ="SELECT ID_N as Id ,max(Use_nome) as utilizadores , Est_nome as estado, Not_titulo as Titulo, Not_texto as Noticia, Tip_categ as categoria, max(EstN_data) as Data " +
  "FROM utilizador " +
...
// filtra as noticias por utilizador para o historico do moderador so as enviadas
  ...
  @Query(value= "SELECT ID_N as id ,max(Use_nome) as utilizadores , max(Est_nome) as estado, Not_titulo as Titulo, Not_texto as Noticia, Tip_categ as categoria,max(EstN_data) as Data " +
  "FROM utilizador " +
  "inner join estadonoticia on EsTN_ID_U =ID_U " +
  "inner join noticias on EsTN_ID_N =ID_N " +
  "inner join estado  on EsTN_ID_E=ID_E " +
...
"group by ID_N " +
"order by Data desc limit 6 ",nativeQuery = true)
